...
function DeliveredByMeScreen() {
  const { userInfo } = useSelector((state) => state.auth);
  const navigate = useNavigate();
  const { data: orders, isLoading } = useDeliveredByMeQuery(userInfo.user._id);
  return (
    <div>
      <Button
        variant="contained"
      ...
      <Box sx={{ minHeight: 601 }}>
        <Typography variant="h3">Orders Delivered By Me</Typography>
...
export default DeliveredByMeScreen;
